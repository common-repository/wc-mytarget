...
final class IMTFW_Error_Log {
	protected $text_to_log;
	protected $log_dir_name; // /home/site.ru/public_html/wp-content/uploads/integrate-mytarget-for-woocommerce
	protected $log_file_name; // /home/site.ru/public_html/wp-content/uploads/integrate-mytarget-for-woocommerce/plugin.log

	public function __construct($text_to_log, $log_dir_name = IMTFW_PLUGIN_UPLOADS_DIR_PATH) {
	...
	protected function keeplogs_status() {
		if (is_multisite()) {
			$v = get_blog_option(get_current_blog_id(), 'imtfw_keeplogs');
		} else {
			$v = get_option('imtfw_keeplogs');
		}	
		if ($v === 'on') {
			return true;
		} else {
